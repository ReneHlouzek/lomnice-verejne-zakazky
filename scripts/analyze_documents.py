#!/usr/bin/env python3
"""Extract neutral, factual signals from retained PDF text."""
from __future__ import annotations
import json,re
from pathlib import Path

ROOT=Path(__file__).resolve().parents[1]
MANIFEST=ROOT/"data/documents/registr-smluv/manifest.json"
...
def main():
    ...
    for rec in obj.get("records",[]):
        for d in rec.get("documents",[]):
            ...
            cats,scores=classify(d.get("name",""),text)
            amounts=[]
            ...
            docs.append({
                "source":"registr-smluv","source_id":rec.get("source_id"),
                "title":rec.get("title"),"document_name":d.get("name"),
                "text_file":d["text_file"],"text_chars":len(text),
                "document_types":cats,"signals":scores,
                "amounts_czk":sorted(set(amounts))[:30],
                "dates":sorted(set(DATE.findall(text)))[:40],
                "snippets":snippets(text),
            })
    OUT.write_text(json.dumps({
        "version":1,"documents_analyzed":len(docs),
